...
This project implements a tiny system on chip. It has a 16 bit microprocessor, a boot rom, a PWM, a UART, an SPI and a timer protocol.

The boot rom has 32 words. After reset it runs a program to get input from outside and display to outside. The program has all the instructions that this processor supports. This tapeout is done to test the microprocessor on silicon. The SPI, PWM, UART and timers are memory mapped. The processor writes the data to SPI, PWM, UART and timers so that those IPs can be tested also.
